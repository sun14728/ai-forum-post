...
function httpGet(url) {
  return new Promise(function(resolve, reject) {
    ...
    mod.get(url, { headers: { "User-Agent": "Mozilla/5.0" }, timeout: 15000 }, function(res) {
...
function extractFromReasoning(reasoning) {
  ...
  while ((m = quoteRegex.exec(reasoning)) !== null) {
    ...
    if (text.length >= 20 && zhCount / text.length > 0.3) {
      candidates.push({ text: text, pos: m.index });
    }
  }
  ...
  for (var i = lines.length - 1; i >= 0; i--) {
    ...
    for (var j = 0; j < line.length; j++) {
      if (line.charCodeAt(j) >= 0x4e00 && line.charCodeAt(j) <= 0x9fff) zhCount++;
    }
    if (zhCount / Math.max(line.length, 1) > 0.3 && line.length >= 20) {
      candidates.push({ text: line, pos: i * 10000 + 5000 });
    }
  }
  if (candidates.length > 0) {
    candidates.sort(function(a, b) { return b.pos - a.pos; });
    var best = candidates[0].text;
    best = best.replace(/^\*{1,2}[^:]*:\*{1,2}\s*/g, "");
    best = best.replace(/\*{1,2}$/g, "");
    return best.trim();
  }
  return "";
}
...
async function getThreads() {
  try {
    var resp = await httpGet(CONFIG.forum.url + "/api_threads.php?limit=30");
    var data = JSON.parse(stripBom(resp));
    if (data.code === 0 && data.threads) return data.threads;
    return [];
  } catch(e) { log("getThreads failed: " + e.message); return []; }
}
...
async function generateReply(thread, bot) {
  var prompt = "你是一个论坛用户，昵称叫\"" + bot.name + "\"，风格特点：" + bot.style + "\n" +
    "请针对这个帖子写一条中文回复（80-200字）。要求：全部使用中文，禁止输出英文单词句子（技术专有名词除外）。\n" +
    "帖子标题：" + thread.subject + "\n" +
  ...
  try {
    ...
    log("  LLM no choices");
    return null;
  } catch(e) { log("  LLM failed: " + e.message); return null; }
}
...
async function postReply(tid, message, bot) {
  ...
  } catch(e) { log("  Reply API error: " + e.message); return null; }
}
...
async function main() {
  ...
  var targets = eligible.slice(0, 5);
  var totalReplies = 0;
  ...
  for (var ti = 0; ti < targets.length; ti++) {
    ...
    var needed = Math.max(0, targetReplies - existing);
    ...
    for (var i = 0; i < needed; i++) {
      ...
      bot = available.length > 0 ? available[Math.floor(Math.random() * available.length)] : CONFIG.bots[Math.floor(Math.random() * CONFIG.bots.length)];
      usedBots.add(bot.uid);
      log("  [" + (i + 1) + "/" + needed + "] " + bot.name);

      var reply = await generateReply(thread, bot);
      if (!reply || reply.length < 10) { log("    skip (empty)"); continue; }
